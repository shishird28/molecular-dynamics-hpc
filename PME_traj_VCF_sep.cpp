...
string filename = "PME_traj_N4_T300_2000.xyz";
string filename2 = "Force_N4_alpha1.dat";
string filenameNa = "VCF_N8_T300_Na_v2.dat";
string filenameCl = "VCF_N8_T300_Cl_v2.dat";
string filename4 = "velocity_N4.dat";
...
/*	double sigma_Cl = 3.85;
   ...
   void charge_assign(vector<vector<double>> pos) {
    ...
    double VACF(vector<vector<double>> v) {
	double vcf = 0;
	for (int i = 0; i < total; i++) {
	    double vx = v[i][0];
	    double vy = v[i][1];
...
int main(int argc, char **argv) {
    ...
    int N = 10000; // iterations
    //int v_N = N/1000;
    int v_N = 1;
    double dt = 50; // time step

    double temp = 300; // Temperature in Kelvin
    double v = sqrt(temp/5.65e9); // velocity magnitude in au

    vector<array<double, 10000>> vcf_Na;
    vector<array<double, 10000>> vcf_Cl;
...
/*	    if (f.rank == 0) {
	...
	double vcf_j_Cl = 0;
	    ...
	    if (f.rank==0) {
	        double vsq = v0_list[j][0]*vx + v0_list[j][1]*vy + v0_list[j][2]*vz;
                
		if (f.q_list[j] < 0) vcf_j_Cl += vsq; 
	        else vcf_j_Na += vsq;	
	    }	    
	}
	...
	if (f.rank == 0) {
	    vcf_j_Na /= (size*0.5);
	    vcf_j_Cl /= (size*0.5);
            vcf_Na[v_idx][v_iter] = vcf_j_Na;
	    vcf_Cl[v_idx][v_iter] = vcf_j_Cl;
	    //vel_file << v_list[50][0] << "\n";
	}
    }  
    if (f.rank == 0) {
	double v_norm = 0;
        std::ofstream vcf_file_Na(filenameNa);
	std::ofstream vcf_file_Cl(filenameCl);
	for (int j = 0; j < v_split; j++) {
	    double v_avg_Na = 0;
	    double v_avg_Cl = 0;
	    for (int i = 0; i < v_N; i++) {
		v_avg_Na += vcf_Na[i][j];
	    ...
	    v_avg_Na /= v_N;
	    v_avg_Cl /= v_N;
            if (j==0) v_norm = v_avg_Na;
            v_avg_Na /= v_norm;
            v_avg_Cl /= v_norm;	    
            //vcf_file_Na << t_list[j] << " ";
            //vcf_file_Cl << t_list[j] << " ";	    
	    
	    vcf_file_Na << v_avg_Na << "\n";
	    vcf_file_Cl << v_avg_Cl << "\n";
        }
    }
